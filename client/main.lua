...
Citizen.CreateThread(function() -- tags  
    while true do
        ...
        local ped = PlayerPedId()
            local yacht = AddBlipForCoord(-938.44, -2932.28, 13.95)
                SetBlipSprite(yacht, 16)
...
Citizen.CreateThread(function()
  while true do
    Citizen.Wait(1)
      local ped = PlayerPedId()
        if heist == false then
        if GetDistanceBetweenCoords(GetEntityCoords(ped), -938.44, -2932.28, 13.95, true) < 50 then
                if GetDistanceBetweenCoords(GetEntityCoords(ped), -938.44, -2932.28, 13.95, true) < 1 then
                        if IsControlJustReleased(1, 51) then
                            TriggerServerEvent("rcAirportRobbery:robbery")
                        end
                    end
                end
            end
    ...
    exports['qtarget']:AddTargetModel({1809430156}, {
        options = {
            {
                event = "rcAirportRobbery:start",
                icon = "fas fa-chart-bar",
                label = "Zapocni Pljacku",
            },
        },
        distance = 2.0
    })
    ...
    RegisterNetEvent('rcAirportRobbery:start')
    AddEventHandler('rcAirportRobbery:start', function()
    
            rcAirportRobbery:start() 
    end)
...
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(1)
            local ped = PlayerPedId()
            if GetDistanceBetweenCoords(GetEntityCoords(ped), -938.44, -2932.28, 13.95, true) > 100 then
                heist = false
                tags = false
            end
        end
    ...
    while true do 
      Citizen.Wait(1)
        ...
        if location1 == false and tags == true then
            DrawMarker(20, -938.44, -2932.28, 13.95, 0, 0, 0, 0, 0, 155.0, 1.0, 1.0, 1.0, 0, 155, 253, 155, 0, 0, 2, 0, 0, 0, 0)
            if GetDistanceBetweenCoords(GetEntityCoords(ped), -938.44, -2932.28, 13.95, true) < 1 then
        ...
        if location2 == false and tags == true then
            DrawMarker(20, -939.44, -2932.28, 13.95, 0, 0, 0, 0, 0, 155.0, 1.0, 1.0, 1.0, 0, 155, 253, 155, 0, 0, 2, 0, 0, 0, 0)
            if GetDistanceBetweenCoords(GetEntityCoords(ped), -938.44, -2932.28, 13.95, true) < 1 then
                ESX.ShowHelpNotification(_U('search_robbery'))
                    if IsControlJustReleased(1, 51) then
                        Animacja()
        ...
        if location3 == false and tags == true then
            DrawMarker(20, -938.44, -2933.28, 13.95, 0, 0, 0, 0, 0, 155.0, 1.0, 1.0, 1.0, 0, 155, 253, 155, 0, 0, 2, 0, 0, 0, 0)
        ...
        if location4 == false and tags == true then
            DrawMarker(20, -938.48, -2933.30, 13.95, 0, 0, 0, 0, 0, 70.0, 1.0, 1.0, 1.0, 0, 155, 253, 155, 0, 0, 2, 0, 0, 0, 0)
...
Citizen.CreateThread(function()
    while searched < 20 do 
        Citizen.Wait(100)
            if searched == 20 then
                TriggerServerEvent("rcAirportRobbery:reward")
                heist = false
                tags = false
                Citizen.Wait(19000)
...
RegisterNetEvent("rcAirportRobbery:notification")
AddEventHandler("rcAirportRobbery:notification", function(text)
    ESX.ShowHelpNotification(text)
end)

RegisterNetEvent("rcAirportRobbery:notify")
AddEventHandler("rcAirportRobbery:notify", function(text)
    ESX.ShowNotification(_U('notification'))
    PlaySoundFrontend(-1, "HACKING_SUCCESS", 0, 1)
end)

RegisterCommand("neq", function()
    TriggerEvent("rcAirportRobbery:lspd")
end)
